...
- An important part of code review is backing up your opinions with reference 
  to documentation, formal guidelines, or common design patterns. It's not 
  enough to just state your opinion that a change should be made. You also 
  need to provide some context for the recommendation. Additionally, you should
  avoid using jokes or sarcasm in code reviews. These can be easily 
  misunderstood out of context.
  
- We have an awful lot of computers here, and it gets pretty confusing with
  slightly different things running on all of them. How could containers help us
  improve this situation?
- There is a consistent problem in software engineering that code runs fine
  on one developers local machine, but doesn't on another developers machine.
  Virtual environments are useful for making sure that different people can 
  have the exact same dependencies. However, containers take this one step 
  further and ensure that software will run no matter where it is deployed. 
  This is because a container is a minimal virtual operating system, and has 
  all of the software necessary to run an application.
